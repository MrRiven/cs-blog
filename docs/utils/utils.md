 ...
 * @param {fileName} 文件名
 */
export function downLoadInPost(content, fileName, downType = 'application/vnd.ms-excel') {
...
export function FloatDivide(num1, num2) {
  ...
  // 默认保留小数点最长的个数
  return Math.ceil((integer1 / integer2) * Math.pow(10, length2 - length1) * 100) / 100
}
```
